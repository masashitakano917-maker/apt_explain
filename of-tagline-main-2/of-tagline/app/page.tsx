...
function markDiffRed(original: string, improved: string) {
  ...
  while (i < n && j < m) {
    if (A[i] === B[j]) {
      ...
      out.push(`<mark class="bg-orange-50 text-orange-700">${B[j++]}</mark>`);
    }
  }
...
async function callReview(
  ...
  const improved =
    ...
    (j?.text_after_check &&
      typeof j.text_after_check === "string" &&
      j.text_after_check.trim()) ||
    draftText ||
    "";

  const issuesArr = (Array.isArray(j?.issues_structured)
    ? j?.issues_structured
...
async function callPolish(
  ...
  const j = await safeJson<{ ok?: boolean; text?: string; notes?: string[] }>(
    "/api/polish",
    {
      method: "POST",
      headers: { "content-type": "application/json" },
...
function StepDot({ state }: { state: StepState }) {
  const base =
    "w-6 h-6 rounded-full flex items-center justify-center border select-none";
  if (state === "done")
  ...
  if (state === "active")
    return (
      <div
        className={cn(
          base,
          "bg-orange-500/90 border-orange-600 text-white animate-pulse"
        )}
      >
        ✓
      </div>
...
export default function Page() {
  ...
  const stepsForTracker = [
    {
      key: "draft" as StepKey,
      label: "ドラフト",
      sub: draftStep === "active" ? "作成中" : draftStep === "done" ? "完了" : "",
      state: draftStep,
    },
    {
      key: "check" as StepKey,
      label: "安全チェック",
      sub: checkStep === "active" ? "実行中" : checkStep === "done" ? "完了" : "",
      state: checkStep,
    },
    {
      key: "polish" as StepKey,
      label: "仕上げ提案",
      sub: polishStep === "active" ? "生成中" : polishStep === "done" ? "完了" : "",
      state: polishStep,
    },
  ];
              ...
              <div className="grid grid-cols-2 gap-3">
                <label className="flex flex-col gap-1">
                  ...
                  <input
                    ...
                    max={2000}
                    onChange={(e) =>
                      setMinChars(clamp(Number(e.target.value || 450), 200, 2000))
                    }
                  />
                </label>
                <label className="flex flex-col gap-1">
                  ...
                  <input
                    ...
                    max={2000}
                    onChange={(e) =>
                      setMaxChars(clamp(Number(e.target.value || 550), 200, 2000))
                    }
                  />
                </label>
                <div className="col-span-2 text-xs text-neutral-500">
                  推奨：450〜550　|　現在：{minChars}〜{maxChars}　|　最新本文長：
                  {jaLen(currentText)} 文字
                </div>
              </div>
              ...
              <details className="rounded-lg border bg-neutral-50 p-3">
                ...
                <div className="grid grid-cols-2 gap-3 mt-3">
                  ...
                  <label className="flex flex-col gap-1">
                    ...
                    <input
                      className="border rounded p-2"
                      placeholder="例）1998年築 / 1998年10月築"
                      value={built}
                  ...
                  <label className="flex flex-col gap-1 col-span-2">
                    ...
                    <input
                      className="border rounded p-2"
                      placeholder="例）長期修繕計画あり"
                      value={maintFeeNote}
                      onChange={(e) => setMaintFeeNote(e.target.value)}
                    />
                  </label>
                </div>
              </details>
              ...
              <div className="space-y-3">
                {!!diff12Html && (
                  <details open className="rounded border">
                    <summary className="cursor-pointer px-3 py-2 text-sm bg-neutral-50">
                      差分（ドラフト → 安全チェック済）
                    </summary>
                    <div
...
// …以下、右ペイン/フッタはあなたの最新版と同じ
        ...
        <section className="space-y-4">
          ...
          <div
            ref={draftRef}
            className="bg-white rounded-2xl shadow min-h-[220px] flex flex-col overflow-hidden scroll-mt-24"
          >
            <div className="p-4 border-b flex items-center justify-between">
              <div className="text-sm font-medium">ドラフト</div>
              <div className="flex items-center gap-3">
                <div className="text-xs text-neutral-500">長さ：{jaLen(text1)} 文字</div>
                <Button onClick={() => copy(text1)} disabled={!text1}>
                  コピー
                </Button>
              </div>
            </div>
          ...
          >
            <div className="p-4 border-b flex items-center justify-between">
              ...
              <div className="flex items-center gap-3">
                ...
                <Button onClick={() => copy(text2)} disabled={!text2}>
                  コピー
                </Button>
              </div>
            </div>
            ...
            <div className="p-4 border-b flex items-center justify-between">
              ...
              <div className="flex items-center gap-3">
                <div className="text-xs text-neutral-500">
                  長さ：{jaLen(text3 || text2)} 文字
                </div>
                <Button onClick={() => copy(text3 || text2)} disabled={!(text3 || text2)}>
                  コピー
                </Button>
              </div>
            </div>
          ...
          <div className="bg-white rounded-2xl shadow p-4">
            <div className="text-xs text-neutral-500 leading-relaxed">
              ※ ドラフトは <code>/api/describe</code>、安全チェックは{" "}
              <code>/api/review</code>、仕上げは <code>/api/polish</code> を使用。<br />
              仕上げでは不足文字数を補い、トーン/文流れを整えます。左の「言い換えサジェスト」からの追記も可能です。
            </div>
          </div>
        </section>
      </main>
